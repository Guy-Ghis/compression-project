...
# --- Configuration ---
RUST_IMPL_DIR="/home/guy_ghis/projects/holiday_assignments/compression_tool/rust-compressor" # Assuming the Rust implementation is in a directory named rust-impl
JS_IMPL_DIR="/home/guy_ghis/projects/holiday_assignments/compression_tool/js-compressor"     # Assuming the JS implementation is in a directory named js-impl
RUST_EXECUTABLE="$RUST_IMPL_DIR/target/release/rust-compressor" # Adjust if needed
...
RUST_COMPRESSED_FILE="$OUTPUT_DIR/$(basename "$INPUT_FILE").rs.compressed.$COMP_ALGORTIHM"
RUST_DECOMPRESSED_FILE="$OUTPUT_DIR/$(basename "$INPUT_FILE").rs.decompressed.$COMP_ALGORTIHM"
JS_COMPRESSED_FILE="$OUTPUT_DIR/$(basename "$INPUT_FILE").js.compressed.$COMP_ALGORTIHM"
JS_DECOMPRESSED_FILE="$OUTPUT_DIR/$(basename "$INPUT_FILE").js.decompressed.$COMP_ALGORTIHM"
...
/usr/bin/time -f "$TIME_FORMAT" -o "$TIME_TMP_FILE" "$RUST_EXECUTABLE" --algorithm "$COMP_ALGORTIHM" compress "$INPUT_FILE" "$RUST_COMPRESSED_FILE"
if [ $? -ne 0 ]; then
...
/usr/bin/time -f "$TIME_FORMAT" -o "$TIME_TMP_FILE" "$RUST_EXECUTABLE" --algorithm "$COMP_ALGORTIHM" decompress "$RUST_COMPRESSED_FILE" "$RUST_DECOMPRESSED_FILE" 
if [ $? -ne 0 ]; then
...
/usr/bin/time -f "$TIME_FORMAT" -o "$TIME_TMP_FILE" node --max-old-space-size=8192 "$JS_SCRIPT" -- compress "$INPUT_FILE" "$JS_COMPRESSED_FILE" -a "$COMP_ALGORTIHM"
if [ $? -ne 0 ]; then
...
/usr/bin/time -f "$TIME_FORMAT" -o "$TIME_TMP_FILE" node --max-old-space-size=8192 "$JS_SCRIPT" -- decompress "$JS_COMPRESSED_FILE" "$JS_DECOMPRESSED_FILE" -a "$COMP_ALGORTIHM"
if [ $? -ne 0 ]; then
